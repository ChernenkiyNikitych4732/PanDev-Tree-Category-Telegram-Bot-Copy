...
@Component
public class AddRootElementCommand implements BotCommand {

    private static final Logger logger = LoggerFactory.getLogger(AddRootElementCommand.class);

    private final CategoryService categoryService;

    public AddRootElementCommand(CategoryService categoryService) {
    ...
    public void execute(DefaultAbsSender sender, Update update) {
        ...
        try {
            if (parts.length == 2) {
                ...
                response = "Пустой корневой элемент";
            }
        } catch (Exception e) {
